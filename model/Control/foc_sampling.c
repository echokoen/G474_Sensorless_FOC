...
static void foc_sampling_update_currents(FOC_SamplingData_t *sampling)
{
  ...
  if ((sampling->fallback_request != 0u) && (sampling->last_valid_current_ready != 0u))
  {
    ...
    if (sampling->fallback_hold_count < 0xFFFFu)
    {
      sampling->fallback_hold_count++;
    }
    return;
  }
#endif

  sampling->fallback_hold_count = 0u;
...
uint8_t FOC_SamplingModule_IsHoldLastTooLong(const FOC_SamplingData_t *sampling)
{
  ...
  return (sampling->fallback_hold_count >= FOC_ADC_FALLBACK_HOLD_MAX_TICKS) ? 1u : 0u;
}
