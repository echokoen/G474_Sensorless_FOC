 ...
 *
 * 0 = 仍使用开环角；
 * 1 = 开环角和 observer 角做平滑混合；
 * 2 = 完全使用 observer 角。
 */
#define FOC_SW_STATE_OPENLOOP  (0u)
#define FOC_SW_STATE_BLEND     (1u)
#define FOC_SW_STATE_OBSERVER  (2u)

static volatile uint8_t s_sw_state = FOC_SW_STATE_OPENLOOP;          /* 当前控制角来源状态。 */
static volatile uint8_t s_observer_ready = 0u;                       /* observer 是否已满足接管准备条件。 */
static volatile uint32_t s_sw_hold_ticks = 0u;                       /* 接管条件连续满足计数。 */
static volatile uint32_t s_sw_blend_ticks = 0u;                      /* 角度混合阶段已经运行的拍数。 */
static volatile float s_sw_blend_k = 0.0f;                           /* 角度混合系数，0=开环角，1=observer 角。 */
...
static inline float foc_openloop_elec_speed_rad_s(void)
{
  const float two_pi = 6.2831853071795864769f;
  return two_pi * s_openloop_mech_freq_hz * FOC_POLE_PAIRS;
}
...
static void foc_reset_switchover(void)
{
  s_sw_state = FOC_SW_STATE_OPENLOOP;
  s_observer_ready = 0u;
  s_sw_hold_ticks = 0u;
  s_sw_blend_ticks = 0u;
  s_sw_blend_k = 0.0f;
...
static uint8_t foc_observer_switchover_condition(void)
{
  const float open_speed = foc_openloop_elec_speed_rad_s();
  const float speed_err = fabsf(s_obs_speed_rad_s - open_speed);
  ...
  if (s_openloop_mech_freq_hz < FOC_SWITCHOVER_MIN_MECH_FREQ_HZ)
  {
    return 0u;
  }

  if (fabsf(s_obs_angle_err_deg) > FOC_SWITCHOVER_MAX_ANGLE_ERR_DEG)
  {
    return 0u;
  }
...
static void foc_update_switchover_state(void)
{
  ...
  const uint8_t ready_now = foc_observer_switchover_condition();

  if (s_sw_state == FOC_SW_STATE_OPENLOOP)
  {
    if (ready_now != 0u)
    {
      if (s_sw_hold_ticks < hold_ticks)
      {
        s_sw_hold_ticks++;
      }
      else
  ...
  else if (s_sw_state == FOC_SW_STATE_BLEND)
  {
    if (ready_now == 0u)
    {
      foc_reset_switchover();
    }
    else if (blend_ticks == 0u)
    {
      s_observer_ready = 1u;
      s_sw_blend_k = 1.0f;
      s_sw_state = FOC_SW_STATE_OBSERVER;
    }
    else if (s_sw_blend_ticks < blend_ticks)
    {
      s_observer_ready = 1u;
      s_sw_blend_ticks++;
      s_sw_blend_k = (float)s_sw_blend_ticks / (float)blend_ticks;
    }
    else
    {
      s_observer_ready = 1u;
...
static float foc_select_control_theta(void)
{
  float theta_ctrl = s_openloop_theta_e_rad;

  if (s_sw_state == FOC_SW_STATE_BLEND)
  {
    const float delta = foc_wrap_pi(s_obs_theta_rad - s_openloop_theta_e_rad);
    theta_ctrl = foc_wrap_2pi(s_openloop_theta_e_rad + (s_sw_blend_k * delta));
  }
  else if (s_sw_state == FOC_SW_STATE_OBSERVER)
  {
    theta_ctrl = foc_wrap_2pi(s_obs_theta_rad);
  }
...
static void foc_observer_update(float valpha, float vbeta)
{
  ...
  s_obs_theta_rad = foc_wrap_2pi(s_flux_obs.Etheta_O + FOC_OBS_THETA_COMP_RAD); /* 补偿后的 observer 电角度。 */
...
void FOC_OpenLoopRun(void)
{
  ...
  foc_update_switchover_state(); /* 根据上一拍 observer 结果决定本拍控制角是否开始接管。 */
  const float theta_ctrl = foc_select_control_theta(); /* 当前拍真正用于 Park / 反 Park 的控制角。 */
...
{
  if (sw_state) *sw_state = s_sw_state;
  if (observer_ready) *observer_ready = s_observer_ready;
  if (blend_k) *blend_k = s_sw_blend_k;
  if (theta_open) *theta_open = s_openloop_theta_e_rad;
  if (theta_obs) *theta_obs = s_obs_theta_rad;
  if (theta_ctrl) *theta_ctrl = s_theta_ctrl_rad;
  if (angle_err_deg) *angle_err_deg = s_obs_angle_err_deg;
  if (open_speed_rad_s) *open_speed_rad_s = foc_openloop_elec_speed_rad_s();
  if (obs_speed_rad_s) *obs_speed_rad_s = s_obs_speed_rad_s;
}
