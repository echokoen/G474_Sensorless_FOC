#ifndef FOC_HANDOVER_H
#define FOC_HANDOVER_H
#ifdef __cplusplus
extern "C" {
#endif
...
typedef enum
{
  FOC_HANDOVER_STATE_OPENLOOP = 0, /* 仍使用开环角作为控制角。 */
  FOC_HANDOVER_STATE_BLEND,        /* 开环角和 observer 角按 blend_k 平滑混合。 */
  FOC_HANDOVER_STATE_OBSERVER      /* observer 角完全接管控制角。 */
} FOC_HandoverState_t;
...
typedef struct
{
  ...
  FOC_HandoverState_t state; /* 当前接管状态机状态。 */
} FOC_Handover_t;
