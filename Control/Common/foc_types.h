 ...
 * 还是“应用层运行总览”，分别放到 FOC_TraceSnapshot_t
...
typedef struct
{
  ...
  float angle_err_deg;   /* 兼容字段：开环角与观测角误差，来自 handover。 */
  ...
  float vbus_v;          /* 当前母线电压。 */
  uint8_t locked;        /* 兼容字段：observer 基础锁定状态，来自 handover。 */
...
typedef FOC_TraceSnapshot_t FocTraceSnapshot_t;
